...
download.file(url = "https://s3-sa-east-1.amazonaws.com/ckan.saude.gov.br/SRAG/2020/INFLUD-11-10-2021.csv",
              destfile = temp)
...
download.file(url = "https://s3-sa-east-1.amazonaws.com/ckan.saude.gov.br/SRAG/2021/INFLUD21-11-10-2021.csv",
              destfile = temp)
...
doses2 = readPNG("doses2.png")

casosBrasil_final0 = casosBrasil_final |>
  filter(state=="TOTAL")
...
```{r}
casosBrasil_final0$`Média móvel` <- round(casosBrasil_final0$MM_casos,0)
casosBrasil_final0$`Casos novos` <- casosBrasil_final0$newCases
casosBrasil_final0$Data <- casosBrasil_final0$date

g1 <- casosBrasil_final0 |> ggplot(aes(x=Data, y=`Casos novos`))+
...
```{r}
casosBrasil_final0$`Média móvel` <- round(casosBrasil_final0$MM_obitos,0)
casosBrasil_final0$`Óbitos novos` <- casosBrasil_final0$newDeaths

g2 <- casosBrasil_final0 |> ggplot(aes(x=Data, y=`Óbitos novos`))+
  geom_bar(stat="identity",
           fill="orange")+
  geom_line(aes(y=`Média móvel`),
            col="blue")+
...
```{r}
casosBrasil_final0$`Leitos clínicos ocupados` <- casosBrasil_final0$ocupacaoConfirmadoCli
casosBrasil_final0$`média móvel` <- casosBrasil_final0$MM_ocup_Cli

g7 <- casosBrasil_final0 |> ggplot(aes(x=Data, y=`Leitos clínicos ocupados`))+
  geom_bar(stat="identity",
           fill="orange")+
  geom_line(aes(y=`média móvel`),
            col="blue")+
...
```{r}
casosBrasil_final0$`Leitos UTI ocupados` <- casosBrasil_final0$ocupacaoConfirmadoUti
casosBrasil_final0$`média móvel` <- casosBrasil_final0$MM_ocup_Uti

g8 <- casosBrasil_final0 |> ggplot(aes(x=Data, y=`Leitos UTI ocupados`))+
  geom_bar(stat="identity",
           fill="orange")+
  geom_line(aes(y=`média móvel`),
            col="blue")+
...
```{r}
casosBrasil_final0$`Primeira dose` <- casosBrasil_final0$D1
casosBrasil_final0$`Segunda dose ou dose única` <- casosBrasil_final0$D2

g3 <- casosBrasil_final0 |>
...
```{r}
casosBrasil_final0$`Vacinas aplicadas` <- casosBrasil_final0$D1

g4 <- casosBrasil_final0 |>
  filter(date>="2021-01-01") |>
...
```{r}
casosBrasil_final0$`Vacinas aplicadas` <- casosBrasil_final0$D2

g5 <- casosBrasil_final0 |>  
  filter(date>="2021-01-01") |>
  ggplot(aes(x=Data, y=`Vacinas aplicadas`))+
  geom_bar(stat="identity",
           fill="red")+
...
```{r}
casosBrasil_final0$`Vacinas aplicadas` <- casosBrasil_final0$D3

g20 <- casosBrasil_final0 |>  
  filter(date>="2021-01-01") |>
  ggplot(aes(x=Data, y=`Vacinas aplicadas`))+
  geom_bar(stat="identity",
           fill="red")+
...
#casosBrasil_final0$populacao= 213317639


#Acumulado das vacinas aplicadas
casosBrasil_final2= casosBrasil_final0 |>
  filter(date>= "2021-01-01")
casosBrasil_final2$D1= as.character(casosBrasil_final2$D1)
for (i in 1:length(casosBrasil_final2$D1)){
  if(is.na(casosBrasil_final2$D1[i]==T)) casosBrasil_final2$D1[i]="Nao informado"
}
casosBrasil_final2= dplyr::filter(casosBrasil_final2, D1!="Nao informado")
casosBrasil_final2$D1= as.numeric(casosBrasil_final2$D1)

casosBrasil_final2$D1_AC= cumsum(casosBrasil_final2$D1)
casosBrasil_final2$D2_AC= cumsum(casosBrasil_final2$D2)
casosBrasil_final2$D3_AC= cumsum(casosBrasil_final2$D3)

casosBrasil_final2$D2_AC= as.character(casosBrasil_final2$D2_AC)
for (i in 1:length(casosBrasil_final2$D2_AC)){
  if((casosBrasil_final2$date[i]<"2021-01-17")) casosBrasil_final2$D2_AC[i]=NA
}
casosBrasil_final2$D2_AC= as.numeric(casosBrasil_final2$D2_AC)

casosBrasil_final2$D3_AC= as.character(casosBrasil_final2$D3_AC)
for (i in 1:length(casosBrasil_final2$D3_AC)){
  if((casosBrasil_final2$date[i]<"2021-08-03")) casosBrasil_final2$D3_AC[i]=NA
}
casosBrasil_final2$D3_AC= as.numeric(casosBrasil_final2$D3_AC)
...
#BaseBrasil_aux = casosBrasil_final2
...
```{r }
casosBrasil_final2$`Aplicações primeira dose` <- casosBrasil_final2$D1_AC
casosBrasil_final2$`Aplicações segunda dose ou dose única` <- casosBrasil_final2$D2_AC
casosBrasil_final2$`Aplicações terceira dose` <- casosBrasil_final2$D3_AC

g6 <- casosBrasil_final2 |>
...
g1 = ggplot(base_atual) +
  ...
  xlab('') +
  ylab('') +
  ggtitle('Casos \n Para os 10 estados com mais casos') +
  ...
  annotation_custom(rasterGrob(img),
                    xmin=paste(base_atual$Estados[base_atual$CasosAcumulados == sort(base_atual$CasosAcumulados)[2]][1]),
                    xmax=paste(base_atual$Estados[base_atual$CasosAcumulados == sort(base_atual$CasosAcumulados)[1]]),
...
```{r fig.align='center', echo=FALSE, message=FALSE, warning=FALSE, fig.width = 12}
base_topo = data.frame(x = fct_reorder(base_atual$Estados, base_atual$CasosAcumulados), y = base_atual$letalidade)
...
g4 = ggplot(base_atual) +
  ...
  theme(axis.title.x = element_blank(),
        ...
        plot.title = element_text(size=9))+
  annotation_custom(rasterGrob(img),
                    xmin=paste(base_atual$Estados[base_atual$tx_casos_mil == sort(base_atual$tx_casos_mil)[3]]),
                    xmax=paste(base_atual$Estados[base_atual$tx_casos_mil == sort(base_atual$tx_casos_mil)[1]]),
...
g5 = ggplot(base_atual) +
  ...
  theme(axis.title.x=element_blank(),
        ...
        plot.title = element_text(size=9))+
  annotation_custom(rasterGrob(img),
                    xmin=paste(base_atual$Estados[base_atual$tx_mort_mil == sort(base_atual$tx_mort_mil)[3][1]]),
                    xmax=paste(base_atual$Estados[base_atual$tx_mort_mil == sort(base_atual$tx_mort_mil)[1]]),
...
```{r fig.align='center', echo=FALSE, message=FALSE, warning=FALSE, fig.width = 10}
#load("casosBrasil_final.rdata")
casosBrasil_final2= casosBrasil_final |>
  select(Data, Estados, D1, D2, D3)

casosBrasil_final2$D1= as.character(casosBrasil_final2$D1)
for (i in 1:length(casosBrasil_final2$D1)){
  if(is.na(casosBrasil_final2$D1[i]==T)) casosBrasil_final2$D1[i]=0
}
casosBrasil_final2$D1= as.numeric(casosBrasil_final2$D1)

for (i in 1:length(casosBrasil_final2$D2)){
  if(is.na(casosBrasil_final2$D2[i]==T)) casosBrasil_final2$D2[i]=0
}
casosBrasil_final2$D2= as.numeric(casosBrasil_final2$D2)

for (i in 1:length(casosBrasil_final2$D3)){
  if(is.na(casosBrasil_final2$D3[i]==T)) casosBrasil_final2$D3[i]=0
}
casosBrasil_final2$D3= as.numeric(casosBrasil_final2$D3)

#Data_hoje = max(casosBrasil_final$Data)

casosBrasil_final2 = casosBrasil_final2 |>
  group_by(Estados) |>
...
#aux_inconsistencia = str_to_upper(aux_inconsistencia)
bra2 = left_join(bra2_map, BaseBrasil_aux, by = c("nome" = "Estados"))
...
estados = estados |>
  left_join(casosBrasil_final, by= "ibgeID") |>
  filter(Data == max(Data)) |>
  mutate(Estados = str_to_upper(Estados))

bra2 = bra2 |>
  left_join(estados, by= c("Estados","Data"))

bra2 = bra2 |>
  mutate(taxaD1= round(D1_AC/populacao, 2),
         taxaD2= round(D2_AC/populacao, 2),
         taxaD3= round(D3_AC/populacao, 2),
         rotulo1= str_c(Estados, " - ", paste0(round(taxaD1,2)*100, "%")),
         rotulo2= str_c(Estados, " - ", paste0(round(taxaD2,2)*100, "%")),
         rotulo3= str_c(Estados, " - ", paste0(round(taxaD3,2)*100, "%")))
...
ggplot(casosBrasil_final |> filter(Estados %in% base_atual$Estados), aes(num_dias_caso,tx_casos_mil,group= Estados)) +
  geom_line(aes(colour = factor(Estados), linetype = Estados, size = Estados), show.legend = FALSE) +
 scale_linetype_manual(values = line) +
  scale_size_manual(values = size) +
  ylab("Taxa de casos confirmados (por 100.000 habitantes)") +
  xlab(paste("Dias transcorridos após registro do", paste(N_dias_casos, "º", sep=""), "caso")) +
  ggtitle(data_hoje) +
  geom_label_repel(
    data= base_atual |> filter(Data == max(Data)), # Filter data first
    aes(label=Estados, fill = Estados),
    point.padding = unit(0.15, "lines"), # Rectangle size around label
    show.legend = FALSE,
    segment.size = 0.35,
    color = "black"
  ) +
  ...
  annotation_custom(rasterGrob(img),
                    xmin = 0, xmax = 30,
                    ymin = max((casosRJ_final |> filter(Estados %in% base_atual$Estados))$tx_casos_mil) * 0.8,
                    ymax = max((casosRJ_final |> filter(Estados %in% base_atual$Estados))$tx_casos_mil) * 1.1)
